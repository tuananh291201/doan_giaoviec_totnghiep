...
const ChangeTaskActions = ({ _id, stage, user }) => {
  const [changeStage] = useChangeTaskStageMutation();
  const [postActivity] = usePostTaskActivityMutation();

  const changeHanlder = async (val) => {
    try {
      // Nhân viên không tự chuyển sang Hoàn thành; thay vào đó gửi yêu cầu duyệt
      if (val === "completed" && !user?.isAdmin) {
        const res = await postActivity({
          data: {
            type: "completed",
            activity: "Yeu cau quan ly duyet hoan thanh",
            notiType: "alert",
            text: "Nhan vien yeu cau duyet hoan thanh cong viec",
          },
          id: _id,
        }).unwrap();
        toast.success(res?.message || "Da gui yeu cau duyet hoan thanh cong viec");
        return;
      }
          ...
          <CustomTransition>
            <Menu.Items className='absolute p-4 right-0 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none'>
              ...
              <div className='px-1 py-1'>
                <Menu.Item>
                  <ChangeTaskActions id={task._id} {...task} user={user} />
                </Menu.Item>
              </div>
